...
db.addRace(name="Wizard", id = 20, faction = "Alliance", playableClasses = "Hunter", description = "Od" )
...
role = db.getRole("Tank")
assert(role is not None)
assert(db.getRoles()[0] is role)
db.addRole(name="Gun", description = "Od")
assert(len(db.getRoles()) == 4)
gun = db.getRole("Gun")
assert(gun is not None)
assert(db.getRoles()[3] is gun)
db.commit()
db.deleteRole(gun)
gun = db.getRole("Gun")
assert(gun is None)
...
print("   Testing Stat methods...")
assert(len(db.getStats()) == 12)
stat = db.getStat("2v2 Info")
assert(stat is not None)
assert(db.getStats()[0] is stat)
db.addStat(name="5v5 Info", stat = "od", description = "Od")
assert(len(db.getStats()) == 13)
new_stat = db.getStat("5v5 Info")
assert(new_stat is not None)
assert(db.getStats()[12] is new_stat)
db.commit()
db.deleteStat(new_stat)
new_stat = db.getStat("5v5 Info")
...
assert(contents['info'] == "The /2v2arena endpoints in this API provide different statistics and information based on the top 5000 players in the WoW 2v2 ladder.")
...
contents = get_json(r)
assert('info' in contents)
assert(contents['info'] == "The /3v3arena endpoints in this API provide different statistics and information based on the top 5000 players in the WoW 3v3 ladder.")
...
contents = get_json(r)
assert('info' in contents)
assert(contents['info'] == "The /RBG endpoints in this API provide different statistics and information based on the top 5000 players in the WoW RBG ladder.")
